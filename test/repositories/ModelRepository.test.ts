...
describe('ModelRepository Tests', () => {
  ...
  describe('loadModelsConfig() method', () => {
    ...
    test('should return valid config structure', () => {
      ...
      Object.entries(config).forEach(([providerId, providerConfig]) => {
        assert.ok(typeof providerConfig === 'object', `Config for ${providerId} should be object`)
        assert.ok('name' in providerConfig, `Config for ${providerId} should have name`)
        assert.ok('models' in providerConfig, `Config for ${providerId} should have models`)
        assert.ok('defaultModel' in providerConfig, `Config for ${providerId} should have defaultModel`)
        
        assert.ok(typeof providerConfig.name === 'string', `Name for ${providerId} should be string`)
        assert.ok(Array.isArray(providerConfig.models), `Models for ${providerId} should be array`)
        assert.ok(typeof providerConfig.defaultModel === 'string', `Default model for ${providerId} should be string`)
    ...
    test('should have consistent provider data', () => {
      ...
      if ('anthropic' in config) {
        ...
        assert.ok(anthropicConfig.models.some(model => model.includes('claude')))
      }
    })
  ...
  describe('validateModelsConfig() method', () => {
    ...
    test('should reject providers with missing required fields', () => {
      ...
      const configMissingName = {
        ...
        anthropic: { name: 'Anthropic', models: ['claude'], defaultModel: 'claude' },
        google: { name: 'Google', models: ['gemini'], defaultModel: 'gemini' },
        deepseek: { name: 'DeepSeek', models: ['deepseek'], defaultModel: 'deepseek' },
        openrouter: { name: 'OpenRouter', models: ['openrouter'], defaultModel: 'openrouter' }
      }
      assert.throws(() => {
  ...
  describe('Integration with DI Container', () => {
    test('should be resolvable from DI container', async () => {
      ...
      const repository = container.resolve<FileModelRepository>('ModelRepository')
