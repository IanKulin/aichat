...
describe("SqliteChatRepository Tests", () => {
  ...
  describe("Conversation CRUD operations", () => {
    it("should create a new conversation", async () => {
      ...
      const conversation =
        await repository.createConversation(conversationData);
      ...
      assert.ok(
        ...
        "Should have updatedAt date"
      );
      assert.strictEqual(
        conversation.createdAt.getTime(),
        conversation.updatedAt.getTime(),
        "Created and updated times should be equal initially"
      );
    });
    ...
    it("should throw error when updating non-existent conversation title", async () => {
      await assert.rejects(
        async () =>
          await repository.updateConversationTitle("non-existent", "New Title"),
  ...
  describe("Message CRUD operations", () => {
    ...
    it("should update conversation timestamp when saving message", async () => {
      ...
      const updatedConversation =
        await repository.getConversation(conversationId);
      assert.ok(
        updatedConversation!.updatedAt > originalUpdatedAt,
        "Conversation updated timestamp should be newer"
      );
    });
  ...
  describe("Transaction handling", () => {
    ...
    it("should maintain data consistency during conversation deletion", async () => {
      ...
      const deletedConversation = await repository.getConversation(
        conversation.id
      );
  ...
  describe("Cleanup operations", () => {
    it("should delete conversations older than threshold", async () => {
      ...
      const oldConv = await repository.createConversation({
      ...
      const recentConv = await repository.createConversation({
    ...
    it("should cascade delete messages with old conversations", async () => {
      ...
      const conv = await repository.createConversation({
    ...
    it("should delete multiple old conversations", async () => {
      ...
      assert.strictEqual(
        remainingCount,
        2,
        "Should have 2 recent conversations remaining"
      );
    });
