...
describe('Error Handling Tests', () => {
  describe('Error message sanitization', () => {
    const sanitizeError = (error: Error): string => {
      let clientError = "Sorry, I'm having trouble processing your request right now."
      ...
      if (error.message.includes("API key")) {
        ...
        clientError = "Selected model is not available. Please try a different model."
      }
    ...
    test('should sanitize API key errors', () => {
      ...
      assert.strictEqual(result, 'Service configuration error. Please try again later.')
    })
    ...
    test('should sanitize API errors', () => {
      ...
      const result = sanitizeError(error)
      assert.strictEqual(result, 'AI service is temporarily unavailable. Please try again.')
    })

    test('should sanitize model errors', () => {
      const error = new Error('Model error: Invalid model specified')
      const result = sanitizeError(error)
      assert.strictEqual(result, 'Selected model is not available. Please try a different model.')
    })

    test('should return default error for unknown errors', () => {
      const error = new Error('Some unknown internal error')
      const result = sanitizeError(error)
      assert.strictEqual(result, "Sorry, I'm having trouble processing your request right now.")
    })
    ...
    test('should not expose internal error details', () => {
      const error = new Error('Internal server error with sensitive data: password123')
      const result = sanitizeError(error)
      assert.strictEqual(result, "Sorry, I'm having trouble processing your request right now.")
