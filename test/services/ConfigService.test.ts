  ...
  describe('getProviderConfig() method', () => {
    ...
    test('should return configurations for all supported providers', () => {
      ...
      providers.forEach(provider => {
        ...
        assert.ok(Array.isArray(config.models), `Models should be array for ${provider}`)
        assert.ok(config.defaultModel, `Default model should exist for ${provider}`)
        assert.ok(config.models.includes(config.defaultModel), `Default model should be in models array for ${provider}`)
      })
    })
    ...
    test('should load configuration during construction', () => {
      ...
      const testRepository = {
        loadModelsConfig: () => {
          loadCalled = true
          return {
            test: { name: 'Test', models: ['test-model'], defaultModel: 'test-model' }
          }
        },
